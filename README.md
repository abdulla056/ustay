...
# 3. Seed a dev user (owner@ustay.test / ustay-dev-password) — see docs/DATABASE.md#seeding
bun run db:seed
...
| `bun run db:seed`     | Seed local/dev data (idempotent)        |
| `bun run db:reset`    | Drop, recreate, and reseed the local db |
